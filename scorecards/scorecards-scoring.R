...
scores <- scorecard %>% 
    ...
    mutate(across(c(machine:history), 
                  ~ case_when(.x == "Yes" ~ 2, 
                              .x == "No" ~ 0)), 
           across(c(cases_residents:population_staff), 
                  ~ case_when(.x == "Facility-Level" ~ 2,
                              .x == "Statewide" ~ 1, 
                              .x == "" ~ 0)), 
           across(c(machine:population_staff), 
                  ~ replace(., is.na(.), 0))) %>% 
    rename_with(~paste0(., "_score"), c(machine:population_staff)) %>% 
